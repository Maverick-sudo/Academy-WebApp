...
```js
variableName = (condition-true/false) ? value1 : value2
```
...
Functions often compute a return value. The return value is "returned" back to the "caller" using a return statement. To pass back information from the function call, we use a return statement. To create a return statement, we use the return keyword preceded by the code that we wish to execute. If the line of code is omitted, undefined is returned instead. When a return statement is used in a function body, the execution of the function is stopped and the code that follows it will not be executed.
...
```
asyncFuncExample(); // invokes/calls the async function
...
// Creating an asynchronous function using an arrow expression and saving it to the variable asyncFunction.
...
Math.random() * x // returns a random number between 0 (inclusive) and x (exclusive) - value less than x
Math.random() * x + 1 // returns a random number between 0 (inclusive) and x (inclusive) - value less than or equal to x
...
Note: JSON.stringify will not stringify functions. This can be fixed if you convert the functions into strings using `toString()` with the function as an argument before stringifying.
...
If a client requests a route path using the GET HTTP method, the server responds (res.send) with the captured values corresponding to the req.params object.
...
| `^` | Start of The String - Caret Sign ^:`<br>`Selecting by Line Start | `"^hello"` |
| `$` | End of The String - Dollar Sign $:`<br>`Selecting by End of Line | `"world$"` |
...
css("propertyname") - returns the value of the specified property  
css("propertyname","value") - sets the style attribute & properties for the selected elements to the second parameter -"value"  
`css({"propertyname":"value","propertyname":"value",...})` - sets multiple CSS properties using an object with curly braces.
