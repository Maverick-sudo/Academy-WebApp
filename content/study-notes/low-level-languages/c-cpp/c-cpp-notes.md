...
```mermaid
flowchart TD
    Start["Expression: 5/9"] --> Check{"Any operand\nfloating point?"}
    Check -->|No| IntDiv["Integer Division\nResult: 0\n(truncated)"]
    Check -->|Yes| FloatDiv["Floating Point Division\nResult: 0.555..."]
...
```mermaid
flowchart TD
    Start["factorial(4) called"] --> Frame1["Stack Frame 1\nn=4\ncalls factorial(3)"]
    Frame1 --> Frame2["Stack Frame 2\nn=3\ncalls factorial(2)"]
    Frame2 --> Frame3["Stack Frame 3\nn=2\ncalls factorial(1)"]
    Frame3 --> Frame4["Stack Frame 4\nn=1\nBase case: return 1"]
...
flowchart TD
    subgraph "Stack Growth - Multiple Function Calls"
        ...
        FuncA["funcA() Stack Frame\nLocal: x, y\nReturn addr: main+offset\nFP: 0xFFEC\nSP: 0xFFE0"]
        FuncB["funcB() Stack Frame\nLocal: a, b, c\nReturn addr: funcA+offset\nFP: 0xFFDC\nSP: 0xFFD0"]
        FuncC["funcC() Stack Frame\nLocal: temp\nReturn addr: funcB+offset\nFP: 0xFFCC\nSP: 0xFFC8"]
    end
    ...
    Return1["funcC() returns\nSP restored to 0xFFD0\nPop frame"]
    Return2["funcB() returns\nSP restored to 0xFFE0\nPop frame"]
    Return3["funcA() returns\nSP restored to 0xFFF0\nPop frame"]
...
flowchart LR
    subgraph "Caller Responsibilities"
        Caller1["Save caller-saved\nregisters if needed"]
        Caller2["Push arguments\nonto stack"]
        Caller3["Call function\n(saves return address)"]
        Caller4["Restore caller-saved\nregisters"]
        Caller5["Clean up stack\n(pop arguments)"]
    end
    
    subgraph "Callee Responsibilities"
        Callee1["Save callee-saved\nregisters"]
        Callee2["Allocate stack frame\n(local variables)"]
        Callee3["Execute function\nbody"]
        Callee4["Restore callee-saved\nregisters"]
        Callee5["Return\n(pops return address)"]
    end
