...
**Assembly ↔ Machine Code ↔ Execution Relationship:**

```mermaid
graph TB
    subgraph Human["Human-Readable Layer"]
        ASM[Assembly Language<br/>MOV EAX, 5<br/>ADD EAX, 3<br/>RET]
    end
    
    subgraph Translation["Translation Layer"]
        Assembler[Assembler<br/>Translates mnemonics<br/>to opcodes]
        Disassembler[Disassembler<br/>Translates opcodes<br/>to mnemonics]
    end
    
    subgraph Machine["Machine Code Layer"]
        OpCode[Operation Codes OpCodes<br/>0xB8 05 00 00 00<br/>0x83 C0 03<br/>0xC3]
        Hex[Hexadecimal<br/>Representation<br/>B805000000]
        Binary[Binary Bits<br/>10111000 00000101 00000000...]
    end
    
    subgraph Execution["Execution Layer"]
        CPU[CPU Processor<br/>Fetch-Decode-Execute Cycle]
        Registers[Registers<br/>EAX, EBX, ECX, EDX<br/>Instruction Pointer EIP/RIP]
        Memory[Physical Memory RAM<br/>Code, Data, Stack, Heap]
    end
    
    ASM -->|Assembler| Assembler
    Assembler --> OpCode
    OpCode -.->|Disassembler| Disassembler
    Disassembler -.-> ASM
    ...
    note1[Example Transformation:<br/>MOV EAX, 5 → 0xB8 05 00 00 00<br/>Each instruction has unique opcode<br/>Platform-specific x86, x64, ARM]
    
    note2[CPU Operation:<br/>1. Fetch instruction from memory<br/>2. Decode opcode<br/>3. Execute operation<br/>4. Store result in register/memory]
    
    style Human fill:#e6f3ff,stroke:#0078D4,stroke-width:3px
    style Translation fill:#fff4e6,stroke:#F39C12,stroke-width:3px
    style Machine fill:#ffe6e6,stroke:#E74C3C,stroke-width:3px
    style Execution fill:#e6ffe6,stroke:#2ECC71,stroke-width:3px
    style ASM fill:#4A90E2,color:#fff
    style OpCode fill:#E74C3C,color:#fff
    style CPU fill:#2ECC71,color:#fff
    style note1 fill:#FFF9C4,stroke:#F39C12
    style note2 fill:#FFF9C4,stroke:#2ECC71
```

*Figure: Assembly is human-readable representation of opcodes (machine code). Assemblers translate mnemonics to binary; disassemblers reverse the process. CPU fetches opcodes from memory, decodes them, and executes operations on registers.*
...
**Compilation Pipeline - From Source to Execution:**

```mermaid
flowchart TD
    Start[Developer writes<br/>High-Level Source Code<br/>C, C++, Java, Python, etc.]
    
    subgraph Traditional["Traditional Compilation (C/C++/Rust)"]
        Compile1[Compiler/Assembler<br/>Translates to machine code]
        Obj1[Object Code<br/>Platform-specific<br/>CPU-dependent]
        Link1[Linker<br/>Combines object files]
        Exec1[Executable Binary<br/>.exe, .elf, .mach-o]
        CPU1[CPU<br/>Direct Execution<br/>Native performance]
        
        Start -->|C/C++ code| Compile1
        Compile1 --> Obj1
        Obj1 --> Link1
        Link1 --> Exec1
        Exec1 --> CPU1
    ...
    subgraph Bytecode["Bytecode Compilation (Java/.NET)"]
        Compile2[Compiler<br/>Translates to bytecode]
        ByteCode[Bytecode<br/>Platform-independent<br/>.class, .dll]
        VM[Virtual Machine<br/>JVM / CLR]
        JIT[JIT Compiler<br/>Bytecode → Machine Code]
        CPU2[CPU<br/>Execution via VM<br/>Slight overhead]
        
        Start -->|Java/C# code| Compile2
        Compile2 --> ByteCode
        ByteCode --> VM
        VM --> JIT
        JIT --> CPU2
    ...
    subgraph Interpreted["Interpreted (Python/JavaScript)"]
        Interp[Interpreter<br/>Reads source directly]
        Parse[Parser<br/>Analyzes syntax]
        Execute[Execute line-by-line<br/>Or compile to bytecode<br/>behind the scenes]
        CPU3[CPU<br/>Execution via interpreter<br/>More overhead]
        
        Start -->|Python/JS code| Interp
        Interp --> Parse
        Parse --> Execute
        Execute --> CPU3
    end
    
    Rev1[Disassembler<br/>Reverse: Binary → Assembly]
    Rev2[Decompiler<br/>Reverse: Binary → High-level code]
    
    Exec1 -.->|Reverse Engineering| Rev1
    Exec1 -.->|Reverse Engineering| Rev2
    ByteCode -.->|Decompile .class| Rev2
    
    style Traditional fill:#e6f3ff,stroke:#0078D4,stroke-width:3px
    style Bytecode fill:#fff4e6,stroke:#F39C12,stroke-width:3px
    style Interpreted fill:#e6ffe6,stroke:#2ECC71,stroke-width:3px
    style CPU1 fill:#E74C3C,color:#fff
    style CPU2 fill:#E74C3C,color:#fff
    style CPU3 fill:#E74C3C,color:#fff
    style Rev1 fill:#9B59B6,color:#fff
    style Rev2 fill:#9B59B6,color:#fff
```

*Figure: Three compilation models - Traditional (direct to CPU-specific machine code), Bytecode (platform-independent VM execution), and Interpreted (runtime parsing). Reverse engineering uses disassemblers and decompilers to recover source from binaries.*
...
**Debugger Workflow - Breakpoints & Single Stepping:**

```mermaid
flowchart TD
    Start[Launch Target Program<br/>Under Debugger Control]
    
    Attach[Debugger Attaches<br/>Gains control of process]
    
    SetBP{Set Breakpoint?}
    SetBPYes[Set Breakpoint at Address<br/>0x00401000: SUB ESP, 0x20<br/>Debugger replaces instruction<br/>with INT 3 opcode 0xCC]
    
    Run[Continue Execution<br/>Program runs normally<br/>until breakpoint hit]
    
    HitBP{Breakpoint Hit?}
    
    Break[CPU executes INT 3<br/>Generates Debug Exception<br/>OS notifies debugger]
    
    Pause[Execution PAUSED<br/>Debugger gains control]
    
    Examine[Examine Program State:<br/>✓ Registers EAX, EBX, ESP, EIP<br/>✓ Stack contents<br/>✓ Memory dump<br/>✓ Disassembly window<br/>✓ Call stack backtrace]
    
    Action{Choose Action}
    
    StepOver[Step Over F10<br/>Execute current instruction<br/>Skip over function calls<br/>Stop at next line]
    
    StepInto[Step Into F11<br/>Execute current instruction<br/>Follow into function calls<br/>Stop at first instruction inside]
    
    StepOut[Step Out Shift+F11<br/>Execute until function returns<br/>Stop after RET instruction]
    
    Continue[Continue F5<br/>Resume normal execution<br/>until next breakpoint]
    
    ModifyState[Modify State:<br/>• Change register values<br/>• Modify memory<br/>• Skip instructions<br/>• Jump to different address]
    
    RestoreBP[Debugger restores<br/>original instruction<br/>at breakpoint location]
    
    SingleStep[CPU executes ONE instruction<br/>Trap Flag set in EFLAGS<br/>Generates single-step exception]
    
    UpdateUI[Debugger updates UI:<br/>• Highlight next instruction<br/>• Update register window<br/>• Refresh stack/memory views]
    
    End[Program Terminates<br/>Debugger detaches]
    ...
    Run --> HitBP
    HitBP -->|Yes| Break
    HitBP -->|No - Running| Run
    
    Break --> Pause
    Pause --> Examine
    Examine --> Action
    
    Action -->|Step Over| StepOver
    Action -->|Step Into| StepInto
    Action -->|Step Out| StepOut
    Action -->|Continue| Continue
    Action -->|Modify| ModifyState
    Action -->|Quit| End
    ...
    style Pause fill:#F39C12,color:#000
    style Examine fill:#2ECC71,color:#fff
    style SingleStep fill:#9B59B6,color:#fff
    style End fill:#95A5A6,color:#fff
```

*Figure: Debugger workflow showing breakpoint mechanics (INT 3 replacement), program state inspection, and single-stepping. Debuggers use CPU debug registers and trap flags to control execution. Stepping executes one instruction at a time for detailed analysis.*
...
**Virtual Memory Architecture - Abstraction Layer:**

```mermaid
graph TB
    subgraph Process["Process Virtual Address Space (64-bit: 16 EB)"]
        subgraph KernelSpace["Kernel Space<br/>Accessible ONLY in Kernel Mode"]
            KernelCode[Kernel Code<br/>OS Functions]
            KernelData[Kernel Data<br/>System Structures]
            PagedPool[Paged Pool<br/>Pageable kernel memory]
            NonPagedPool[Non-Paged Pool<br/>Always in RAM]
        end
        
        subgraph UserSpace["User Space<br/>Accessible in User Mode"]
            Stack[Stack<br/>Function calls<br/>Local variables<br/>Grows downward]
            Heap[Heap<br/>Dynamic allocation<br/>malloc/new<br/>Grows upward]
            DataSeg[Data Segment<br/>Global/Static variables]
            Code[Code Segment<br/>.text section]
        end
    end
    
    subgraph MMU["Memory Management Unit MMU"]
        PageTable[Page Tables<br/>Virtual → Physical<br/>Translation]
        TLB[TLB Cache<br/>Fast lookups]
    end
    
    subgraph Physical["Physical RAM"]
        Frame1[Physical Frame 1<br/>4KB page]
        Frame2[Physical Frame 2]
        Frame3[Physical Frame 3]
    end
    
    subgraph Disk["Hard Drive"]
        PageFile[Page File<br/>Swap Storage]
    end
    
    Stack -.->|Virtual Addr| PageTable
    Heap -.->|Virtual Addr| PageTable
    Code -.->|Virtual Addr| PageTable
    
    PageTable -->|Physical Addr| Frame1
    PageTable -->|Physical Addr| Frame2
    PageTable -->|Physical Addr| Frame3
    
    TLB -.->|Cache hit| Frame1
    
    Frame1 <-.->|Page out/in| PageFile
    
    style KernelSpace fill:#E74C3C,stroke:#C0392B,color:#fff,stroke-width:3px
    style UserSpace fill:#4A90E2,stroke:#2980B9,color:#fff,stroke-width:3px
    style MMU fill:#F39C12,stroke:#E67E22,stroke-width:3px
    style Physical fill:#2ECC71,stroke:#27AE60,stroke-width:3px
    style Disk fill:#95A5A6,stroke:#7F8C8D,stroke-width:3px
```

*Figure: Virtual Memory provides abstraction between process address space and physical RAM. MMU translates virtual to physical addresses via page tables. Kernel space is privileged; user space is restricted. Paging allows overflow to disk.*

**Process Memory Layout:**

```mermaid
graph TB
    subgraph Layout["32-bit Process: 4GB Virtual Address Space"]
        High["0xFFFFFFFF<br/>High Memory"]
        
        Kernel["Kernel Space<br/>0xC0000000 - 0xFFFFFFFF<br/>1GB Reserved for OS<br/>Not accessible from user mode"]
        
        Stack1["Stack<br/>0xBFFFFFFF<br/>Thread stacks<br/>Grows DOWNWARD ↓<br/>Local variables<br/>Function parameters<br/>Return addresses"]
        
        Gap1["Unmapped Gap<br/>Guard pages"]
        
        Heap1["Heap<br/>Dynamic memory<br/>malloc, new, VirtualAlloc<br/>Grows UPWARD ↑"]
        
        BSS["BSS Segment<br/>Uninitialized data<br/>Zero-initialized globals"]
        
        Data["Data Segment<br/>Initialized globals<br/>Static variables<br/>Read-write"]
        
        Text["Code .text Segment<br/>0x00400000 typical<br/>Executable instructions<br/>Read-only + Execute<br/>Program code"]
        
        Reserved["Reserved<br/>0x00000000<br/>NULL pointer protection"]
        
        Low["0x00000000<br/>Low Memory"]
    end
    
    High --> Kernel
    Kernel --> Stack1
    Stack1 -.->|Collision Risk<br/>Stack Overflow| Gap1
    Gap1 <-.->|Collision Risk<br/>Out of Memory| Heap1
    Heap1 --> BSS
    BSS --> Data
    Data --> Text
    Text --> Reserved
    ...
    Note1["Growth Direction:<br/>Stack: ↓ High → Low<br/>Heap: ↑ Low → High<br/><br/>If they collide:<br/>Stack Overflow or<br/>Out of Memory error"]
    
    style Kernel fill:#E74C3C,color:#fff
    style Stack1 fill:#9B59B6,color:#fff
    style Heap1 fill:#2ECC71,color:#fff
    style Data fill:#F39C12,color:#000
    style Text fill:#4A90E2,color:#fff
    style Reserved fill:#95A5A6,color:#fff
    style Note1 fill:#FFF9C4,stroke:#F39C12
```

*Figure: Process memory layout showing segment organization. Stack grows downward from high addresses, heap grows upward from low addresses. Code segment is read-only + executable, data is read-write. Kernel space is off-limits to user code.*
...
sequenceDiagram
    ...
    participant MMU as MMU<br/>Page Tables
    participant OS as Memory Manager<br/>Kernel Mode
    participant Disk as Hard Drive<br/>Page File
    participant RAM as Physical RAM
    
    App->>CPU: 1. Access memory address<br/>Read/Write 0x12345000
    CPU->>MMU: 2. Translate virtual address<br/>to physical address
    MMU->>MMU: 3. Check page table entry<br/>Is page present in RAM?
    
    alt Page Present in RAM (Fast Path)
        MMU->>CPU: 4a. Return physical address
        CPU->>RAM: 5a. Access data
        RAM->>App: 6a. Return data (microseconds)
    else Page NOT Present (Page Fault)
        MMU->>CPU: 4b. Generate Page Fault Exception
        CPU->>OS: 5b. Switch to Kernel Mode<br/>Invoke Page Fault Handler
        
        OS->>OS: 6b. Check if address is valid<br/>in Virtual Address Space
        
        alt Valid Address - Page is on Disk
            OS->>RAM: 7b. Find free physical frame<br/>May need to evict another page
            OS->>Disk: 8b. Read page from disk<br/>pagefile.sys / swap
            Disk->>RAM: 9b. Load page into RAM<br/>(milliseconds - 1000x slower)
            OS->>MMU: 10b. Update page table entry<br/>Mark as Present, set physical addr
            OS->>CPU: 11b. Return to User Mode<br/>Resume instruction
            CPU->>App: 12b. Retry memory access<br/>(Now succeeds)
        else Invalid Address - Access Violation
            OS->>App: 7c. Segmentation Fault<br/>Access Violation Exception<br/>Terminate process
        end
    end
    
    Note over App,Disk: Typical Latencies:<br/>RAM access: ~100 nanoseconds<br/>Page fault (disk): ~10 milliseconds<br/>100,000x slower!
```

*Figure: Page faults occur when accessing memory not in RAM. CPU traps to kernel, OS loads page from disk, updates page table, and retries. Valid faults are transparent to applications; invalid addresses cause segmentation faults.*
...
stateDiagram-v2
    ...
    UserMode: User Mode Execution
    UserMode: ✓ Application code runs
    UserMode: ✓ Limited memory access
    UserMode: ✓ No privileged instructions
    UserMode: ✓ Cannot access kernel memory
    UserMode: ✓ Cannot access hardware directly
    
    KernelMode: Kernel Mode Execution
    KernelMode: ✓ OS kernel code runs
    KernelMode: ✓ Full memory access
    KernelMode: ✓ All CPU instructions
    KernelMode: ✓ Access all memory
    KernelMode: ✓ Direct hardware access
    
    UserMode --> KernelMode: System Call (syscall/sysenter)<br/>INT 0x80, SYSCALL instruction<br/>File I/O, Memory alloc, etc.
    UserMode --> KernelMode: Exception/Trap<br/>Page Fault, Division by Zero<br/>Invalid Instruction
    UserMode --> KernelMode: Hardware Interrupt<br/>Timer, Keyboard, Disk I/O<br/>Network packet arrival
    
    KernelMode --> UserMode: Return from System Call (sysret)<br/>IRET instruction<br/>Restore user context
    KernelMode --> UserMode: Exception Handled<br/>Resume or Terminate<br/>Deliver signal to process
    KernelMode --> UserMode: Interrupt Handled<br/>Resume interrupted code<br/>Return to user space
    
    KernelMode --> KernelMode: Nested Interrupts<br/>Higher priority interrupt<br/>Context switch to another process
    
    UserMode --> [*]: Process exits
    KernelMode --> [*]: Process terminated<br/>by kernel
    
    note right of UserMode
        Mode Transition Cost:
        ~100-1000 CPU cycles
        Context save/restore
        Not a full context switch
    end note
...
*Figure: Processes execute in User Mode by default. System calls, exceptions, and interrupts trigger transitions to Kernel Mode. CPU saves user context, executes kernel code with full privileges, then returns to User Mode. This isolation protects the OS from buggy applications.*
