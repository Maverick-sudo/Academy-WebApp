...
High-speed links]
            ...
            AZ1a ---|Low latency
redundant links| AZ1b
            AZ1b ---|Low latency
redundant links| AZ1c
            AZ1c ---|Low latency
redundant links| AZ1a
        end
...
Data Center Cluster]
            ...
            AZ2a ---|Low latency
redundant links| AZ2b
            AZ2b ---|Low latency
redundant links| AZ2c
        end
...
(US-East-1 Region)
    ...
    Edge->>Edge: 7. Cache content locally
(TTL configured)
    Edge->>User: 8. Deliver content
(Latency: ~50ms)
    ...
    Edge->>User: 11. Deliver cached content
(Latency: ~5ms)
    
    Note over Edge: Benefits:
✓ 10x faster delivery
✓ Reduced origin load
✓ Lower latency
```
...
stateDiagram-v2
    ...
    Stopped_Hibernated --> Running: Start Command
(Resume from RAM)
...
10.0.2.0/24]
            ...
            EC2_Pub1[EC2 Instance
Public IP: 52.x.x.x
Private IP: 10.0.1.10]
            EC2_Priv1[EC2 Instance
...
Virtual Network]
        ...
        subgraph AZ1_Azure["Availability Zone 1"]
            PubSub1_Az[Subnet: Frontend
10.1.1.0/24]
            PrivSub1_Az[Subnet: Backend
10.1.2.0/24]
            
            VM_Pub1[VM Instance
Public IP: 52.y.y.y
Private IP: 10.1.1.10]
            VM_Priv1[VM Instance
Private IP: 10.1.2.10]
        end
        
        subgraph AZ2_Azure["Availability Zone 2"]
            PubSub2_Az[Subnet: Frontend
10.1.3.0/24]
            PrivSub2_Az[Subnet: Backend
10.1.4.0/24]
        end
...
graph TB
    subgraph S2S["Site-to-Site VPN"]
        OnPrem1[On-Premises Network
Corporate Datacenter
192.168.0.0/16]
        VPNDevice1[VPN Device/Firewall
Public IP: 203.0.113.10
IPsec Tunnel]
        Internet1((Internet
Encrypted Tunnel))
        VPNGateway1[VPN Gateway
Azure/AWS VPN Endpoint
Public IP: 52.x.x.x]
        Cloud1[Cloud VNet/VPC
10.0.0.0/16
All Subnets]
        
        OnPrem1 --> VPNDevice1
        VPNDevice1 -.->|IPsec/IKE
Encrypted| Internet1
        Internet1 -.->|IPsec/IKE
Encrypted| VPNGateway1
        VPNGateway1 --> Cloud1
        
        note1[Use Case:
✓ Connect entire office
✓ Always-on connection
✓ Site-wide access
✓ Hardware VPN required]
    end
...
Dynamic IP]
        Internet2((Internet
Encrypted Tunnel))
        VPNGateway2[VPN Gateway
Azure/AWS VPN Endpoint
RADIUS/Certificate Auth]
        Cloud2[Cloud VNet/VPC
10.0.0.0/16
Accessible Resources]
        
        Laptop -.->|SSTP/OpenVPN
Certificate Auth| Internet2
        Phone -.->|IKEv2
Certificate Auth| Internet2
...
✓ Software client only]
    ...
    subgraph Hybrid["Hybrid Architecture (Both)"]
        OnPrem3[Corporate Office
Site-to-Site VPN
Always Connected]
        Remote[Remote Employees
Point-to-Site VPN
On-Demand]
        VPNGateway3[Shared VPN Gateway
Handles Both Types]
...
```mermaid
flowchart TD
    Start[Incoming Network Packet
Destination: 10.10.2.15]
    
    Check1{Check Route Table
for Matching Routes}
    
    Longest[Priority 1:
Longest Prefix Match
Most Specific Route]
    Check_Long{Multiple Routes
with Same Prefix?}
    Match_Long[Route: 10.10.2.15/32
✓ Exact match - 32 bits]
    Match_24[Route: 10.10.2.0/24
Less specific - 24 bits]
    
    Static[Priority 2:
Static Routes]
    Check_Static{Static Route
Exists?}
    Static_Types[Static Route Types:
• Internet Gateway
• NAT Gateway
• VPC Peering
• Network Interface
...
Route Exists?}
    ...
    Propagated[Priority 4:
Propagated Routes
Dynamic Routes]
    Prop_Types[Propagated Types:
• BGP from Direct Connect/ExpressRoute
• VPN Static Routes
• VPN BGP Routes
...
VPC/VNet Router]
        DNS1[192.168.1.2
Azure/AWS DNS Server 1
Mapped to cloud DNS]
        DNS2[192.168.1.3
Azure/AWS DNS Server 2
Mapped to cloud DNS]
        Bcast[192.168.1.255
Broadcast Address
Reserved]
